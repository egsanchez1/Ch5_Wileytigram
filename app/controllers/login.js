...
//creates the event handlers for the buttons in the view p 184
...
function showLoginBtnClicked() {
	...
	//swapping which views are visible
	$.createAcctView.hide();
	$.homeView.hide();
	$.loginView.show();
};
...
function faceBookLoginEventHandler(_event) {
	...
	//then calls itself again (after cleaning up) on p. 191
...
function faceBookLoginErrorHandler(_user, _error) {
	...
	// Show the error message somewhere and let the user try again. p. 192
...
function doFacebookLoginAction(_options) {
	...
	//if not logged in, so login is false
	if (FB.loggedIn === false) {
		//COMMENTS TAKEN VIA DR. BABB
...
//this opens the view and gets the controller/view p. 188
...
//responds to the view being closed
$.close = function() {
	$.index.close();
}; 